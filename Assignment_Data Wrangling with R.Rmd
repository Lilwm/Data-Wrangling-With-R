...
ggplot(tenure_df, aes(x = tenure, y = n)) +
  ...
  theme(
    plot.title = element_text(hjust = 0.5, size=16, family="Courier"), 
    axis.text.x = element_text(angle = 45, hjust = 1)
  )
```
...
Based on the results of the analysis, the following conclusions were arrived at:
  ...
  Customers on the fibre optic internet service have the highest churn rate at 75%
  Customers with no online security are more likely to churn
  there is no significant difference between churn rate for those who have entertainment services(movies and TV) and those that do not 
...
In order to create an effective customer retention program, management should take the following measures:
  ...
  Focus more on getting customers to long term contract as they are less likely to churn 
  Review customer feedback on the fibre optic service  since majority of the churned customers have that service
  Explore the option of having online security as an add on for customers to reduce churn rate
  Review why customers with entertainment service are churning at the same rate compared to those that do not have.
